...
public class MahJongLogic {
	...
	List<Operation> pickUp(MahJongState state, List<Integer> playerIds) {
		...
		List<Operation> expectedOperations = ImmutableList.<Operation> of(
				new SetTurn(playerIds.indexOf(playerId)), new Set(M, PU),
				new Set(TAW, newAtWall), new Set(getAtHandKey(playerId),
						newAtHand), new SetVisibility(T + tileIndex,
						ImmutableList.of(playerId)));
	...
	List<Operation> discard(MahJongState state, List<Integer> tilesToDiscard,
		...
		List<Operation> expectedOperations = ImmutableList.<Operation> of(
				...
				new Set(M, D), new Set(TU, newUsed), new Set(
	...
	List<Operation> chi(MahJongState state, List<Integer> tilesToChi,
		...
		List<Operation> expectedOperations = ImmutableList.<Operation> of(
				new SetTurn(playerIds.indexOf(playerId)), new Set(M, C),
				new Set(TU, newUsed),
				new Set(getAtHandKey(playerId), newAtHand), new Set(
						getAtDeclaredKey(playerId), newAtDeclared),
				new SetVisibility(T + tileIndex[0]), new SetVisibility(T
						+ tileIndex[1]), new SetVisibility(T + tileIndex[2]));
		return expectedOperations;
	}
	...
	List<Operation> peng(MahJongState state, List<Integer> tilesToPeng,
		...
		// 5) new SetVisibility(T*, null)
		List<Operation> expectedOperations = ImmutableList.<Operation> of(
				new SetTurn(playerIds.indexOf(playerId)), new Set(M, P),
				new Set(TU, newUsed),
				new Set(getAtHandKey(playerId), newAtHand), new Set(
						getAtDeclaredKey(playerId), newAtDeclared),
				new SetVisibility(T + tileIndex[0]), new SetVisibility(T
	...
	private MahJongState gameApiStateToMahJongState(
		...
		for (int i = 0; i < 4; i++) {
			...
			atDeclared[i] = (List<Integer>) gameApiState
					.get(getAtDeclaredKey(i));
		}

		return new MahJongState(turn, "", ImmutableList.copyOf(playerIds),
				ImmutableList.copyOf(tiles), ImmutableList.copyOf(tilesAtWall),
				ImmutableList.copyOf(tilesUsed),
				ImmutableList.copyOf(atHand[0]),
				ImmutableList.copyOf(atDeclared[0]),
				ImmutableList.copyOf(atHand[1]),
				ImmutableList.copyOf(atDeclared[1]),
				ImmutableList.copyOf(atHand[2]),
				ImmutableList.copyOf(atDeclared[2]),
				ImmutableList.copyOf(atHand[3]),
				ImmutableList.copyOf(atDeclared[3]));
	}
